...
### WS2 result note (Claude Code, 2026-07-03)
**Status: DONE.** research/my_alpha.py (single-function contract, pre-filled with 28d XS top-quintile, runs out of the box) + run_alpha.py + make alpha. The gauntlet per handoff spec: gross headline with net-50bps tradability footnote, regime breakdown (trend + eras), plateau sweep over PARAM_GRID, walk-forward folds, BTC/equal-weight benchmarks, auto-appended trials ledger, research/my_alpha_report.md with a plain-English verdict. Guardrails: weight-contract validation, an automatic look-ahead probe (hides ten trailing weeks, fails loudly on any earlier-weight change), and structural vault withholding (scoring needs --vault, logged as a declared look). docs/ALPHA_SANDBOX.md has the 3-step guide and three worked example ideas. Example run verdict: the founding XS idea posts gross Sharpe +0.68 vs BTC +0.80 on the playground, positive in 11/23 folds; it does not beat buy-and-hold even before costs.
...
### WS-B result note (Claude Code, 2026-07-03)
**Status: DONE.** xmom/alphas.py: nine economically distinct signals behind one pure-function contract (close panel -> trailing signal panel), registry-driven: ts_trend_{10,30,90}, xs_rank_{21,63}, sharpe_mom_30, donchian_55 (close-based channel; panel stores closes only), macd_12_26, mom_accel_30. The funding/term-structure sleeve is absent by design (needs perp funding data; labeled future work, not faked). 14 tests green including a parametrized future-perturbation probe across every alpha in the registry.

### WS-C result note (Claude Code, 2026-07-03)
**Status: DONE.** All nine books run gross on the playground. Market-neutrality confirmed: realized betas 0.001 to 0.023. Standalone Sharpes: donchian_55 +1.38, sharpe_mom_30 +0.86, a +0.30-0.35 cluster (ts_trend_30, macd_12_26, xs_rank_21, mom_accel_30), fast/slow trend ~0. The central finding is the correlation matrix (heatmap committed): the trend/rank/breakout core is 0.6-0.9 internally correlated (ts_trend_30 vs macd 0.91), so nine signals are roughly TWO independent bets; only momentum acceleration diversifies. Pre-registered rule (Sharpe >= 0.30, max |corr| < 0.50) selected donchian_55 + mom_accel_30, rejecting even sharpe_mom_30 (+0.86) at corr 0.83: correlated quality is redundancy. Nine ledger rows; cost-sensitivity note added per brief.

### WS-D result note (Claude Code, 2026-07-03)
**Status: DONE, with the headline prediction falsified and recorded.** Combined book (equal capital over the two vol-targeted sleeves): gross Sharpe +1.13, vol 13.2%, maxDD -13.0% (shallowest in the stable), beta +0.012, turnover netting 1,667% vs 2,056% sleeve mean. The brief's expected result (combined Sharpe > best individual) did NOT hold: +1.13 vs donchian_55's +1.38, because at rho 0.22 the weaker sleeve needed ~+0.78 Sharpe to pay its way and brought +0.30. The blend's real wins: positive Sharpe in BOTH regimes (bull +1.30, bear +0.93; first non-bull-conditional result in the project) and the best Calmar. Validation: 17/23 positive folds; DSR 0.655 at K=79 TOTAL ledger trials (indistinguishable from selection noise, stated); VAULT single look spent: Sharpe +0.49, +5.9% total, maxDD -12.0%, beta -0.010 over 548 days (disaster check passed, no certification claimed). Deliverable: research/ALPHA_RESEARCH_REPORT.md + 4 figures. Note: WS-C and WS-D share one commit because the report artifact spans both; every other workstream committed separately per the brief.
